...
# 通义千问 / Qianwen（阿里云 DashScope）；可选模型含 qwen-plus、qwen3-omni-flash 等
...
**配置方式（.env 优先）**:
- **EMBEDDING_PROVIDER**: `openai` | `qianwen`（不设则：有 DASHSCOPE_API_KEY 用 qianwen，否则 openai）。**说明**：DeepSeek 官方 API 暂无 embeddings 端点，LLM 用 DeepSeek 时需单独配置嵌入（如 `EMBEDDING_PROVIDER=openai` + `OPENAI_API_KEY`）。

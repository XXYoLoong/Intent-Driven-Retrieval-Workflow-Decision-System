...
```json
{
  ...
  "outputs": {
    "result": {
      "step_0": { "result": [{"snippet": "...", "title": "...", "resource_id": "res_doc_xxx"}] },
      "step_1": { "result": "..." }
    }
  },
  "from_cache": false
...
```
含 RETRIEVE 步骤的工作流会在 `outputs.result.step_0.result` 中返回检索到的文档列表（snippet、title、resource_id）。
...
```
若带 tenant_id 未查到，接口会自动再按 run_id 单独查询一次。
...
- `404 Not Found`: 资源或执行记录不存在（如 GET /v1/workflows/runs/{run_id} 未查到 run）
...
Chat 与流式 Chat 发生内部异常时，对用户仅返回统一提示「处理时发生错误，请稍后重试。」（不暴露内部异常信息）；详细堆栈见服务端日志。
...
5. **运行查询 404**: 若按 run_id + tenant_id 未查到执行记录，可省略 tenant_id 再试（接口会先按 tenant_id 查，未果时再仅按 run_id 查）
